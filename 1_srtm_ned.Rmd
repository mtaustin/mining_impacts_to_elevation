...
```{r}
max_elevation_change <- maxValue(az_raster_after_clip)
max_elevation_change

min_elevation_change <- minValue(az_raster_after_clip)
...
```{r}
az_mine_5km_buffer <- az_mine %>%
  sf::st_buffer(dist=5000)
# mapview(az_mine_5km_buffer)
```
...
```{r}
az_mine_5km_reproj <- st_transform(x=az_mine_5km_buffer, crs=4326)
# mapview(az_mine_5km_reproj)
```
...
```{r}
az_5km_unmined <- mask(x=dem_diff, mask=az_mine_5km_reproj, inverse=TRUE)
# mapview(az_5km_mask)
```
...
```{r}
az_mine_wsg84 <- st_transform(az_mine, 4326)

az_mined_dem <- mask(dem_diff, az_mine_wsg84)
mapview(az_mined_dem)
```
...
```{r}
unmined_dem <- getValues(az_5km_unmined)
mined_dem <- getValues(az_mined_dem)
